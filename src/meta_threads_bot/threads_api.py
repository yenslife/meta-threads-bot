...
def login_threads_selenium(username: str, password: str) -> dict[str, str]:
    ...
    try:
        ...
        try:
            # 嘗試找到具有「登入」文字的按鈕
            login_button = driver.find_element(By.XPATH, "//div[contains(text(), '登入')]/ancestor::div[@role='button']")
            print("✅ 找到登入按鈕 (通過文字內容)")
        except Exception:
            try:
                # 嘗試找到表單提交按鈕
                ...
                # 最後嘗試通過角色找到按鈕
                login_button = driver.find_element(By.CSS_SELECTOR, "div[role='button']")
